...
[![Build Status](https://travis-ci.org/lamphc/ng-microservice.svg?branch=master)](https://travis-ci.org/lamphc/ng-microservice)

## 简介

![soa](./src/assets/img/soa.png)
&emsp;&emsp;对于网络应用程序,前端越来越大,我们的网络应用程序中 90％是前端代码.

&emsp;&emsp;随着时间的推移,Web 应用也会发生变化,开发技术和框架也是如此, 这需要支持允许不同的前端框架共存、共用、共生, 前端微服务化应运而生.
...
2. ElementZoneStrategyFactory 工厂函数返回一个 Angular Component 和 Custom Elements 的桥梁策略类,创建和销毁组件引用,同时它还会在 input 改变时触发脏检查,示例:
...
3. createCustomElement 和 customElements.define 分别创建和定义一个自定义元素,其中 customElements.define 第一个参数为最终定义的自定义元素标签名称,示例:
...
4. 在根组件 app.component.ts 中定义微服务对外提供的接口,目前对外提供 5 个接口,2 个事件,如下:
   > 接口说明
...
- 遵循[angular 风格指南](https://angular.cn/guide/styleguide)开发
- Tslint 语法规范
- SonarQube 规范
...
2. 修改 deploy 目录 index.html 对应的 js 引用和 tag 调用,示例:
...
3. 新建 app.translate.ts 文件,配置翻译,目前只配置中文和英文,统一根据微服务命名作为主 key(SOANAME),示例:
...
4. 在根组件 app.component.ts 中引入 TranslateService 和翻译配置文件
...
3. 在根组件 ngOnInit 生命周期中初始化 less 配置文件,其中 lessUrl 会根据环境变量引入对应环境的 less 配置文件,示例:
...
4. 调用 waterTheme 服务 changeTheme 方法切换主题配色
